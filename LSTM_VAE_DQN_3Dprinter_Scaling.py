...
class DynamicThresholdEnv(Env):
    ...
    def step(self, action):
        ...
        # 연속 FN 패널티 강화
        if self.fn_streak > 2:
            reward -= 0.1 * self.fn_streak

        # 최근 Recall 기반 보상/패널티
        recall = self.tp_count / (self.tp_count + self.fn_count) if (self.tp_count + self.fn_count) > 0 else 1.0
        if recall < 0.6:
            reward -= 0.2
        elif recall > 0.75:
            reward += 0.2
        ...
        # Threshold 변화 비용 완화
        threshold_change_cost = abs(self.current_threshold - self.prev_threshold) * 0.005
        reward -= threshold_change_cost
        self.prev_threshold = self.current_threshold

        # 극단적 threshold 경고는 유지하되 완화
        if self.current_threshold >= 0.30:
            reward -= 0.05
        elif self.current_threshold <= 0.02:
            reward -= 0.03
        ...
        if self.current_step % 100 == 0 or done:
            ...
            print(f"  Recall={recall:.3f} FP_streak={self.fp_streak} FN_streak={self.fn_streak}")
